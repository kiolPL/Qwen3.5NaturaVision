...
STAGE2_DIR="${STAGE2_DIR:-${OUTPUT_ROOT}/qwen35-qlora-forest-stage2-rerun}"
...
MIN_FREE_VRAM_MIB="${MIN_FREE_VRAM_MIB:-9500}"
...
check_free_vram() {
  ...
  echo "Free VRAM before Stage 2: ${free_mib} MiB"
}
...
STAGE1_CHECKPOINT="${STAGE1_CHECKPOINT:-$(latest_checkpoint "$STAGE1_DIR")}"
...
if (( STAGE1_GLOBAL_STEP < STAGE1_EPOCHS_INT )); then
  echo "Stage 1 global step ${STAGE1_GLOBAL_STEP} is too small for ${STAGE1_EPOCHS_INT} epoch(s)." >&2
  exit 1
fi

STAGE1_STEPS_PER_EPOCH=$(( STAGE1_GLOBAL_STEP / STAGE1_EPOCHS_INT ))
if (( STAGE1_STEPS_PER_EPOCH == 0 )); then
  echo "Could not infer Stage 1 steps per epoch from checkpoint ${STAGE1_CHECKPOINT}." >&2
  exit 1
fi
...
stage2_args=(
  ...
  --max_steps "$STAGE2_EFFECTIVE_MAX_STEPS"
)

echo "Starting Stage 2 only from ${STAGE1_CHECKPOINT}"
if [[ "$STAGE2_MAX_STEPS" != "-1" ]]; then
  echo "Stage 2 will run ${STAGE2_MAX_STEPS} additional steps (effective max_steps=${STAGE2_EFFECTIVE_MAX_STEPS})"
else
  echo "Stage 2 inferred ${STAGE1_STEPS_PER_EPOCH} steps per epoch from Stage 1"
  echo "Stage 2 will add ${STAGE2_ADDITIONAL_STEPS} steps (effective max_steps=${STAGE2_EFFECTIVE_MAX_STEPS}, total epochs=${STAGE2_TOTAL_EPOCHS})"
fi
echo "Stage 2 target modules: ${TARGET_MODULES}"
if [[ -n "$TARGET_REGEX" ]]; then
  echo "Stage 2 target regex: ${TARGET_REGEX}"
fi
